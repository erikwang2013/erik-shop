...
| 阶段二 P1：统一分账费率 + 卖家分账 | ✅ | SettlementCron 费率源统一到 `payment.gateway_fee.{gateway}` + `payment.platform_rate`（与 webhook 同源，cron.* 仅兼容回退）；新增 MerchantSettlements 写入（order_items→MerchantProducts approved→merchant.commission_rate）；**实测**：162.21 订单 → 平台 5% 佣金 8.11 + stripe 网关费 5.00，卖家 149.97@8% → 佣金 12.00 结算 137.97。Supplier/Affiliate 分账因订单无 supplier/affiliate 关联字段（数据链缺失），记录待 schema 补齐 |
