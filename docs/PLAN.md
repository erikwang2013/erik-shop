...
| 🔴 **新发现全局 bug：HashidsDecode 路由参数解码无效** | ✅ | webman 控制器方法参数来自 findRoute 捕获的原始 hashid（中间件 setParams 不生效）。已统一修复：`BaseApiController::decodedId()` helper + 17 处 {id} 路由方法入口接入（订单/商品/购物车/地址/愿望清单/评价/支付状态/退货/优惠券/通知/对比/退款执行/审核）；**实测**订单详情、商品详情、取消订单、购物车 update/destroy、优惠券领取（hashid 路径）全部通过；顺带修复 Orders 缺 items/logs/documents 关系、Carts 缺 sku 关系 |
| 阶段二 P1：统一分账费率 + 卖家分账 | ✅ | SettlementCron 费率源统一到 `payment.gateway_fee.{gateway}` + `payment.platform_rate`（与 webhook 同源，cron.* 仅兼容回退）；新增 MerchantSettlements 写入（order_items→MerchantProducts approved→merchant.commission_rate）；**实测**：162.21 订单 → 平台 5% 佣金 8.11 + stripe 网关费 5.00，卖家 149.97@8% → 佣金 12.00 结算 137.97 |
| 阶段二 P1：四线分账补全（Supplier/Affiliate） | ✅ | schema 补齐：`erik_products.supplier_id` + `erik_orders.affiliate_link_id`（install.sql + ALTER）；SettlementCron 新增供应商周期结算（按月 upsert SupplierSettlements）+ 分销佣金（AffiliateCommissions + AffiliateLinks 计数）；**实测**：商品 99.98 归供应商当月结算、112.22@10% → 分销佣金 11.22 且链接 orders/commission 更新；AffiliateCommissions 表无 updated_at 补 `$timestamps=false` |
