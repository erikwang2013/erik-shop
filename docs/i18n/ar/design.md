...
### 4.2 قاطع الدائرة وتدهور الخدمة

قاطع دائرة Redis (`app\common\CircuitBreaker`): استدعاءات API الخارجية مثل بوابات الدفع/تسجيل الدخول الاجتماعي تمر جميعها عبر `CircuitBreaker::call()` — 5 إخفاقات متتالية تفتح الدائرة لمدة 30 ثانية، وبعد انتهاء TTL يقوم الطلب التالي تلقائيًا باختبار شبه مفتوح، ويعاد الضبط عند النجاح. القائمة البيضاء لأخطاء الأعمال (بطاقة غير صالحة/رمز غير صالح) لا تُحتسب كفشل، لمنع المهاجمين من إسقاط الخدمة التابعة بطلبات غير صالحة؛ عند تعطل Redis يُسمح بالمرور تلقائيًا. أثناء كسر الدائرة يُرجع الواجهة 503 «الخدمة غير متاحة مؤقتًا».

### 4.3 استخدامات Redis

يُستخدم Redis لتحديد المعدل بدلو الرموز (واجهة `support\Redis`) والتحقق البشري وتخزين Session؛ بيانات الأعمال لا تخضع للتخزين المؤقت على مستوى التطبيق، وتُقرأ مباشرة من MySQL (فصل القراءة/الكتابة + تجمع الاتصالات). يُستخدم Redis أيضًا لنشر حالة تفعيل CDN العامة من لوحة الإدارة إلى service (مفتاح مشترك بادئة `shop:`، TTL 60 ثانية).
