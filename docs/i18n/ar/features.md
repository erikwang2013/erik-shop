...
| **الالتزام العالي** | تحديد المعدل بدلو الرموز (نافذة منزلقة + قواعد 6 نقاط)، فصل القراءة/الكتابة في DB (نسختا قراءة + sticky)، تجمع الاتصالات (DB 50/10 + Redis 30/5)، OPCache (128MB، بيئة Docker) | كامل |
