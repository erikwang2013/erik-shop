...
| **Haute concurrence** | Limitation par seau à jetons (fenêtre glissante + règles sur 6 endpoints), disjoncteur (paiements/connexion sociale, 5 échecs → 30s ouvert + rétablissement semi-ouvert), séparation lecture/écriture DB (2 réplicas de lecture + sticky), pool de connexions (DB 50/10 + Redis 30/5), OPCache (128 Mo, environnement Docker) | Complète |
