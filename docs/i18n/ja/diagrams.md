...
| 第3層 | トラフィック制御 + 依存レジリエンス | RateLimitMiddleware + ブルートフォース Redis カウンター + CircuitBreaker | トークンバケット限流(6エンドポイント) + ログイン/登録の防爆 + 決済/ソーシャルログインのサーキットブレーカー(5回失敗→30s, ハーフオープン復旧) |
...
| 六 | デプロイアーキテクチャ図 | アーキテクチャ図 | Docker Compose のコンテナオーケストレーション、ネットワーク、データボリューム、CDN エッジ層（アップロードボリューム admin_uploads/service_public を含む）を表示 |
| 七 | セキュリティアーキテクチャ図 | アーキテクチャ図 | 6 層の多層防御体系を表示：境界→WAF→トラフィック/レジリエンス(限流+サーキットブレーカー)→認証→データ→レスポンス |
