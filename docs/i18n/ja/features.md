...
| **高並行処理** | トークンバケット限流(スライディングウィンドウ+6エンドポイントルール)、サーキットブレーカー(決済/ソーシャルログイン、5回失敗→30s遮断+ハーフオープン復旧)、DB読み書き分離(2リードレプリカ+sticky)、コネクションプール(DB 50/10+Redis 30/5)、OPCache(128MB, Docker環境) | 完全 |
