...
Redis 서킷 브레이커(`app\common\CircuitBreaker`): 결제 게이트웨이/소셜 로그인 등 외부 API 호출을 통합적으로 `CircuitBreaker::call()` 경유 — 5회 연속 실패 시 30초 차단, TTL 만료 후 다음 요청이 자동으로 반개(半開) 탐지를 수행하고 성공하면 즉시 리셋됩니다. 비즈니스 예외 화이트리스트(유효하지 않은 카드/유효하지 않은 토큰)는 실패로 집계하지 않아, 공격자가 유효하지 않은 요청으로 의존 서비스를 마비시키는 것을 방지합니다. Redis를 사용할 수 없으면 자동으로 통과(fail-open) 처리합니다. 차단 중 인터페이스는 503「서비스를 일시적으로 사용할 수 없음」을 반환합니다.
