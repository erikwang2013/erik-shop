...
<p align="center">
  <img src="../../pet.svg" width="180" alt="Snowy — талисман проекта Erik Shop">
</p>

## Талисман проекта — Snowy

Талисман проекта — **Snowy**: дух-глобус в короне-снежинке, опоясанный меридианами, обнимает посылку. Корона-снежинка символизирует распределённые ID Snowflake, глобус — трансграничную торговлю, посылка — международную логистику. Полное описание: [README на китайском](../../../README.md#项目宠物--雪球-snowy).
...
> Схема охватывает 19 крупных функциональных модулей (включая центр отчётов и статистику платформы).
...
Откройте `http://127.0.0.1:8788/app/admin` в браузере и войдите в панель администратора (при первом использовании создайте учётную запись администратора через мастер установки):

- **Дашборд**: GMV, количество заказов, рост пользователей и другие ключевые показатели
- **Центр отчётов**: сводка продаж, тренд за 30 дней, TOP товары, распределение по способам оплаты/статусам заказов
- Ежедневное управление товарами, заказами, маркетингом, цепочкой поставок и другими модулями
...
# Статистика платформы на стартовой странице (пользователи/товары/заказы/GMV всего и за сегодня)
curl http://127.0.0.1:8787/
```

> Версия API указывается через заголовок `API-Version` (не в URL); для защищённых эндпоинтов требуется `Authorization: Bearer <token>` (JWT).

### Клиенты

- **Flutter-клиент**: `apps/flutter/` (iOS / Android / macOS / Windows / Linux)
- **Клиент HarmonyOS**: `apps/harmonyos/` (HarmonyOS NEXT, ArkTS + ArkUI)
...
shop-php/
  ...
  admin/            админ-панель (webman-admin)   — 83 контроллера + 76 моделей + дашборд ECharts + Web-мастер установки
...
| **Аналитика отчётов** | Центр отчётов админ-панели: сводка продаж, тренд за 30 дней, TOP товары, распределение по способам оплаты/статусам заказов |
| **Статистика платформы** | Статистика на стартовой странице service: пользователи/товары/заказы/GMV всего и за сегодня |
