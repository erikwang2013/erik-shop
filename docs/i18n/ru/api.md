...
// 401 Не авторизован  403 Доступ запрещён  422 Ошибка валидации параметров  429 Слишком много запросов  503 Сервис временно недоступен (предохранитель/деградация)
```
...
URL ресурсов (изображения товаров, карусели, документы) хранятся в БД как относительные пути и на выходных границах переписываются через `Cdn::url()` в `https://{CDN_DOMAIN}{path}`, когда CDN включён (`CDN_ENABLED=true`); при выключенном CDN возвращается исходный относительный путь. Домен CDN по CNAME указывает обратно на домен admin (модель origin-pull), файлы раздаются с иммутабельным кэшем (7 дней) через `/app/admin/upload/`.
