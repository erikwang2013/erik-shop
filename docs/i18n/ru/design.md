...
Redis используется для токен-бакета ограничения частоты (фасад `support\Redis`), счётчиков предохранителя, кодов человеко-машинной проверки и хранения сессий; бизнес-данные не кэшируются на уровне приложения — напрямую читаются из MySQL (разделение чтения/записи + пул соединений). Глобальный вкл/выкл CDN распространяется из admin в service через общий Redis (префикс `shop:`, TTL 60s); статические ресурсы раздаются граничным CDN с иммутабельным кэшем и автоматической очисткой (purge) при CRUD товаров/каруселей, всегда fail-open.
