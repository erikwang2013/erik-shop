...
### 4.2 Использование Redis

Redis используется для токен-бакета ограничения частоты (фасад `support\Redis`), кодов человеко-машинной проверки и хранения сессий; бизнес-данные не кэшируются на уровне приложения — напрямую читаются из MySQL (разделение чтения/записи + пул соединений).
