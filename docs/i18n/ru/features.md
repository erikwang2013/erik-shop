...
| **Высокая нагрузка** | Токен-бакет (скользящее окно + правила 6 конечных точек), разделение чтения/записи БД (2 читающих реплики + sticky), пул соединений (DB 50/10 + Redis 30/5), OPCache (128MB, среда Docker) | Полная |
