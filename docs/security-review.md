...
### 4.1 SecurityGuard 对 webman/Workerman 的 header 依赖（Low Risk）
...
### 4.2 CSRF Origin 检测器对 Admin 的影响（No Risk）
...
### 4.3 31 个检测器全部启用，每请求开销（Performance Note）
...
### 4.4 IP 黑名单持久化（Operational Note）
...
service/.env.example                              (ENCRYPTION_KEY/CIPHER 更新)
service/.env                                      (从 .env.example 新建)
service/CLAUDE.md                                 (中间件栈 + tech stack 更新)
service/composer.json                             (security-php 依赖)
service/config/plugin/erikwang2013/security-php/app.php  (新建, 31 检测器)
service/app/middleware/SecurityMiddleware.php     (重写为使用 SecurityGuard)
docker-compose.yml                                (ENCRYPTION_CIPHER/PREVIOUS_KEYS + admin vars)
```
...
- WAF 检测正确拦截 XSS、SQL 注入等攻击
- 敏感字段加密配置完整（31 个模型，6 类敏感数据）
- API 加解密配置就绪（AES-256-CBC）
- 无功能回归（23/23 测试通过）
- 中间件无重复检测
- Docker 部署环境变量完整
