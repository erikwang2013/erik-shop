...
-- =============================================================
-- 第一部分: webman-admin 管理后台系统表 (wa_ 前缀)
-- =============================================================
...
CREATE TABLE `erik_users` (
    ...
    `last_login_platform` VARCHAR(16) NOT NULL DEFAULT '' COMMENT '最后登录平台',
...
CREATE TABLE `erik_products` (
    ...
    `supplier_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '供应商ID（0=平台自营）',
...
CREATE TABLE `erik_orders` (
    ...
    `platform` VARCHAR(16) NOT NULL DEFAULT 'web' COMMENT '操作来源端',
    `remark` VARCHAR(512) NOT NULL DEFAULT '' COMMENT '订单备注',
    `address_snapshot` JSON COMMENT '地址快照',
    `affiliate_link_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '分销推广链接ID（0=无分销）',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` DATETIME NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
...
CREATE TABLE `erik_payments` (
    ...
    `platform` VARCHAR(16) NOT NULL DEFAULT 'web' COMMENT '支付来源端',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
CREATE TABLE `erik_search_logs` (
    ...
    `platform` VARCHAR(16) NOT NULL DEFAULT 'web' COMMENT '搜索来源端',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    KEY `idx_user_id` (`user_id`),
...
CREATE TABLE `erik_chat_messages` (
    ...
    `platform` VARCHAR(16) NOT NULL DEFAULT 'web' COMMENT '消息来源端',
