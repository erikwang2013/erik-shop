 ...
 *
 * 背景：InstallController 硬编码待安装表清单（含 wa_ 系统表 + erik_ 业务表），
 * install.sql 只含 erik_ 业务表（wa_ 表由 webman-admin 插件自建）。
 * 新增/删除业务表时若只改一处，安装向导的冲突检测与导入就会漂移。
 *
 * 校验规则：
 *   1. install.sql 中的 erik_ 表必须全部出现在 InstallController 清单（防漏装）
 *   2. InstallController 清单中的 erik_ 表必须存在于 install.sql（防多余/改名残留）
...
$ctrlErik = array_values(array_filter($ctrlTables, fn($t) => str_starts_with($t, 'erik_')));
...
$failures = 0;
echo "install.sql erik_ 表: " . count($sqlTables) . "\n";
echo "InstallController erik_ 清单: " . count($ctrlErik) . "（另含 wa_ 系统表 " . count($ctrlTables) - count($ctrlErik) . " 张，插件自建不参与校验）\n";
