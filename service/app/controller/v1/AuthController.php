...
class AuthController extends \app\controller\BaseApiController
{
    ...
    public function register(Request $request): \support\Response
    {
        ...
        if (empty($email) || empty($password)) return ApiResponse::fail('邮箱和密码不能为空', 422);
        if (Users::where('email', $email)->exists()) return ApiResponse::fail('该邮箱已注册', 422);
        $salt = bin2hex(random_bytes(3));
        $user = Users::create(['nickname' => $nickname ?: 'User'.substr(md5($email),0,8), 'email' => $email, 'password' => password_hash($password.$salt, PASSWORD_BCRYPT), 'salt' => $salt, 'invite_code' => strtoupper(substr(md5(uniqid()),0,8)), 'status' => 1]);
        return ApiResponse::success(['user_id'=>HashidsHelper::encode($user->id), 'nickname'=>$user->nickname, 'email'=>$user->email, 'access_token'=>Jwt::encode(['sub'=>(string)$user->id,'email'=>$user->email,'level'=>$user->level]), 'expires_in'=>config('jwt.default_expire',7200)], '注册成功');
    }
    ...
    public function login(Request $request): \support\Response
    {
        ...
        if (!$user || !password_verify($password.$user->salt, $user->password)) return ApiResponse::fail('邮箱或密码错误', 401);
        $user->update(['last_login_at'=>date('Y-m-d H:i:s'), 'last_login_ip'=>$request->getRealIp()]);
        return ApiResponse::success(['user_id'=>HashidsHelper::encode($user->id), 'nickname'=>$user->nickname, 'email'=>$user->email, 'level'=>$user->level, 'access_token'=>Jwt::encode(['sub'=>(string)$user->id,'email'=>$user->email,'level'=>$user->level]), 'expires_in'=>config('jwt.default_expire',7200)], '登录成功');
    }
    ...
    public function refresh(Request $request): \support\Response
    {
        ...
        try {
            ...
            return ApiResponse::success(['access_token'=>Jwt::encode(['sub'=>$payload['sub']??'','email'=>$payload['email']??'','level'=>$payload['level']??0]), 'expires_in'=>config('jwt.default_expire',7200)], 'Token已刷新');
        } catch (\Throwable $e) {
