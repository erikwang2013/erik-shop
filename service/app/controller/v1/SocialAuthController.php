...
class SocialAuthController extends \app\controller\BaseApiController
{
    public function login(Request $request): \support\Response
    {
        ...
        // 社交身份粒度锁：同一 provider_user_id 并发登录会重复绑定/重复注册
        try {
            return DistributedLock::run("lock:social:{$provider}:{$providerUserId}", function () use ($provider, $providerUserId, $email, $name, $request, $verified) {
                // 查找已有绑定
