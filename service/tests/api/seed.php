...
function seedData(): void
{
    ...
    dbInsert('erik_countries', ['id', 'name_en', 'name_cn', 'iso_code_2', 'iso_code_3', 'phone_code', 'currency_code', 'flag_emoji', 'timezone', 'price_display_mode', 'kyc_required', 'status', 'sort'], [
    ...
    dbInsert('erik_currencies', ['id', 'code', 'name', 'symbol', 'is_default', 'status'], [
        [1, 'USD', 'US Dollar', '$', 1, 1],
        [2, 'EUR', 'Euro', '€', 0, 1],
    ...
    dbInsert('erik_exchange_rates', ['id', 'from_currency', 'to_currency', 'rate', 'source', 'effective_at'], [
    ...
    dbInsert('erik_categories', ['id', 'parent_id', 'name', 'slug', 'sort', 'status', 'is_hot', 'level'], [
        [1, 0, 'Clothing', 'clothing', 1, 1, 1, 1],
        [2, 1, 'Dresses', 'dresses', 1, 1, 0, 2],
        [3, 0, 'Shoes', 'shoes', 2, 1, 1, 1],
    ]);

    dbInsert('erik_products', ['id', 'category_id', 'title', 'subtitle', 'slug', 'brand', 'main_image', 'description', 'status', 'is_hot', 'is_new', 'is_recommend', 'sales_count', 'min_price', 'max_price', 'weight', 'unit'], [
    ...
    dbInsert('erik_product_translations', ['product_id', 'locale', 'title', 'subtitle', 'description'], [
    ...
    dbInsert('erik_product_skus', ['id', 'product_id', 'sku_code', 'attrs', 'default_price', 'origin_price', 'cost_price', 'stock', 'status', 'sales_count'], [
    ...
    dbInsert('erik_product_sku_prices', ['sku_id', 'currency_code', 'price', 'origin_price'], [
    ...
    dbInsert('erik_product_images', ['product_id', 'url', 'sort', 'is_main'], [
        [1001, 'https://img.example.com/p1.jpg', 1, 1],
        [1001, 'https://img.example.com/p1b.jpg', 2, 0],
        [1002, 'https://img.example.com/p2.jpg', 1, 1],
    ]);

    dbInsert('erik_hs_codes', ['id', 'code', 'description', 'parent_code', 'section'], [
        [1, '620442', "Women's dresses", '6204', 'Textiles'],
        [2, '640399', 'Footwear', '6403', 'Footwear'],
    ]);
    dbInsert('erik_product_hs_codes', ['product_id', 'hs_code_id', 'is_primary'], [
        [1001, 1, 1],
        [1002, 2, 1],
    ]);
    dbInsert('erik_tariff_rules', ['dest_country_id', 'hs_code_id', 'duty_rate', 'vat_rate', 'duty_free_threshold', 'vat_free_threshold'], [
        [1, 1, 12.000, 0.00, 800.00, 0.00],
        [3, 1, 12.000, 20.00, 135.00, 135.00],
        [4, 1, 12.000, 19.00, 150.00, 150.00],
    ]);
    dbInsert('erik_vat_settings', ['country_id', 'vat_rate', 'reduced_vat_rate', 'ioss_enabled', 'duty_free_threshold', 'vat_free_threshold'], [
    ...
    dbInsert('erik_logistics_companies', ['id', 'name', 'code', 'tracking_url', 'estimated_days', 'status'], [
        [1, 'DHL Express', 'DHL', 'https://www.dhl.com/track?num=', '3-5', 1],
        [2, 'UPS', 'UPS', 'https://www.ups.com/track?num=', '5-8', 1],
    ]);
    dbInsert('erik_shipping_zones', ['id', 'name', 'countries', 'status'], [
        [1, '北美区', '["US","CA"]', 1],
        [2, '西欧区', '["GB","DE"]', 1],
        [3, '亚太区', '["JP"]', 1],
    ]);
    dbInsert('erik_shipping_zone_rates', ['zone_id', 'logistics_id', 'weight_from', 'weight_to', 'price', 'per_kg_price'], [
    ...
    dbInsert('erik_payment_gateways', ['id', 'code', 'name', 'mode', 'status'], [
        [1, 'stripe', 'Stripe', 'sandbox', 1],
        [2, 'paypal', 'PayPal', 'sandbox', 1],
        [3, 'klarna', 'Klarna', 'sandbox', 1],
    ]);
    dbInsert('erik_payment_gateway_methods', ['gateway_id', 'method_code', 'countries', 'currencies', 'min_amount', 'max_amount', 'status'], [
    ...
    dbInsert('erik_banners', ['id', 'title', 'image', 'link_url', 'position', 'countries', 'sort', 'status', 'start_at', 'end_at'], [
    ...
    dbInsert('erik_coupons', ['id', 'title', 'type', 'value', 'min_amount', 'total_qty', 'received_qty', 'per_user_limit', 'scope_type', 'scope_ids', 'countries', 'new_user_only', 'start_at', 'end_at', 'status'], [
    ...
    dbInsert('erik_cms_pages', ['id', 'slug', 'type', 'status', 'published_at'], [
        [1, 'about-us', 'page', 1, $now],
        [2, 'privacy-policy', 'page', 1, $now],
    ]);
    dbInsert('erik_cms_page_translations', ['page_id', 'locale', 'title', 'content'], [
    ...
    dbInsert('erik_faq_translations', ['category', 'locale', 'question', 'answer', 'sort', 'status'], [
    ...
    dbInsert('erik_size_charts', ['id', 'category_id', 'type', 'name'], [
        [1, 1, 'clothing', 'Dress Size Chart'],
    ]);
    dbInsert('erik_size_chart_values', ['chart_id', 'region', 'size_label', 'measurement_cm'], [
        [1, 'US', 'S', 86.00],
        [1, 'US', 'M', 91.00],
        [1, 'EU', '36', 86.00],
        [1, 'EU', '38', 91.00],
    ...
    dbInsert('erik_flash_sales', ['id', 'title', 'start_at', 'end_at', 'status'], [
    ...
    dbInsert('erik_flash_sale_skus', ['flash_sale_id', 'sku_id', 'price', 'stock', 'limit_per_user'], [
        [1, 2001, 19.99, 10, 1],
    ]);
    dbInsert('erik_group_buys', ['id', 'title', 'sku_id', 'group_price', 'required_count', 'expire_hours', 'start_at', 'end_at', 'status'], [
        [1, '2人成团享低价', 2001, 24.99, 2, 24, date('Y-m-d H:i:s', strtotime('-1 hour')), date('Y-m-d H:i:s', strtotime('+24 hours')), 1],
    ]);

    dbInsert('erik_membership_levels', ['id', 'name', 'level', 'min_score', 'icon', 'discount_rate', 'free_shipping', 'status'], [
        [1, 'Bronze', 1, 0, '', 0.00, 0, 1],
        [2, 'Silver', 2, 1000, '', 3.00, 0, 1],
        [3, 'Gold', 3, 5000, '', 5.00, 1, 1],
    ]);
    dbInsert('erik_membership_benefits', ['level_id', 'benefit_type', 'benefit_value'], [
        [2, 'discount', '3%'],
        [3, 'free_shipping', '1'],
        [3, 'priority', '1'],
    ]);

    dbInsert('erik_settings', ['key', 'value', 'group', 'remark'], [
    ...
    dbInsert('erik_gift_cards', ['id', 'code', 'denomination', 'balance', 'currency_code', 'receiver_email', 'message', 'status', 'expire_at'], [
    ...
    dbInsert('erik_warehouses', ['id', 'name', 'country_id', 'address', 'contact', 'is_default', 'is_return_warehouse', 'is_overseas', 'status'], [
        [1, '深圳总仓', 1, 'Shenzhen, CN', 'Ops', 1, 1, 0, 1],
        [2, '德国海外仓', 4, 'Frankfurt, DE', 'DE Ops', 0, 1, 1, 1],
        [3, '日本海外仓', 5, 'Tokyo, JP', 'JP Ops', 0, 0, 1, 1],
    ]);

    dbInsert('erik_notifications', ['id', 'user_id', 'title', 'content', 'type', 'target_type', 'target_id'], [
        [1, 0, '欢迎来到 GlobalShop', '感谢注册，祝您购物愉快！', 'system', '', 0],
        [2, 0, '限时优惠', '全场满100减20', 'promotion', 'page', 1],
    ]);

    dbInsert('erik_compliance_categories', ['id', 'code', 'name', 'description'], [
        [1, 'CE', 'CE标志', '欧盟强制性安全认证'],
        [2, 'FDA', 'FDA认证', '美国食品药品监管'],
        [3, 'RoHS', 'RoHS', '有害物质限制'],
    ]);
    dbInsert('erik_country_compliance_rules', ['country_id', 'compliance_category_id', 'rule', 'restriction_reason'], [
        [1, 2, 'allowed', ''],
        [4, 1, 'allowed', ''],
        [5, 3, 'allowed', ''],
    ]);
    dbInsert('erik_product_compliance', ['product_id', 'compliance_category_id', 'cert_no', 'expire_at'], [
        [1001, 1, 'CE2024001', '2028-01-01'],
    ]);

    dbInsert('erik_users', ['id', 'nickname', 'email', 'password', 'status', 'invite_code'], [
        [1, 'Reviewer', 'reviewer@test.local', 'x', 1, 'REV00001'],
    ]);
    dbInsert('erik_product_reviews', ['id', 'user_id', 'product_id', 'order_id', 'sku_id', 'rating', 'content', 'images', 'status'], [
        [1, 1, 1001, 0, 2001, 5, 'Great dress, fast shipping!', '[]', 1],
        [2, 1, 1001, 0, 2002, 4, 'Nice but runs small.', '[]', 1],
    ]);

    dbInsert('erik_point_rules', ['id', 'code', 'name', 'points', 'limit_type', 'limit_value', 'status'], [
